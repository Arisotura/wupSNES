...
char Mem_ROMPath[256];
char Mem_SRAMPath[256];
...
bool ROM_CheckHeader(u32 offset)
{
    ...
    Flash_Read(ROM_BaseOffset + offset, &chksum, 2);
    Flash_Read(ROM_BaseOffset + offset + 2, &chkcomp, 2);
...
bool Mem_LoadROM(char* path)
{
    ...
    u32 offset, length;
    if (!Flash_GetEntryInfo(path, &offset, &length, NULL))
        return false;

    ROM_BaseOffset = offset;
	ROM_FileSize = length;
    ...
    ROM_HeaderOffset += ROM_BaseOffset;

	u8 sramsize; Flash_Read(ROM_HeaderOffset + 0x18, &sramsize, 1);
	u8 region; Flash_Read(ROM_HeaderOffset + 0x19, &region, 1);
...
void Mem_Reset()
{
    ...
    memset(SPC_IOPorts, 0, sizeof(SPC_IOPorts));
...
inline u8 IO_ReadKeysLow()
{
    ...
    u32 keys = input->ButtonsDown;
    u8 ret = 0;

    if (keys & BTN_A) ret |= 0x80;
    if (keys & BTN_X) ret |= 0x40;
    if (keys & BTN_L) ret |= 0x20;
    if (keys & BTN_R) ret |= 0x10;
...
inline u8 IO_ReadKeysHigh()
{
    ...
    if (keys & BTN_B) 		ret |= 0x80;
    if (keys & BTN_Y) 		ret |= 0x40;
    if (keys & BTN_MINUS)	ret |= 0x20;
    if (keys & BTN_PLUS)	ret |= 0x10;
    if (keys & BTN_UP) 		ret |= 0x08;
    if (keys & BTN_DOWN) 	ret |= 0x04;
    if (keys & BTN_LEFT) 	ret |= 0x02;
    if (keys & BTN_RIGHT) 	ret |= 0x01;
...
void SNES_Divide(u16 num, u8 den, u16* quo, u16* rem)
{
    if (den == 0)
    {
        *quo = 0xFFFF;
	...
	switch (addr)
	{
		...
		case 0x06:
			...
			*(u32*)0x04000298 = (u32)val;
			...
			SNES_Divide(Mem_DivA, val, &Mem_DivRes, &Mem_MulRes);
			break;
