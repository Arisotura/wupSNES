...
void ROM_MapBankToRAM(u32 bank, u8* ptr)
{
	...
	if (Mem_HiROM)
	{
        ...
        Flash_Read(base + ((bank - 0x40) << 16), ptr, 0x10000);
	...
	else
	{
        ...
        Flash_Read(base + (bank << 15), ptr, 0x8000);
...
u16 Mem_ROMRead16(u32 fileaddr)
{
	if (fileaddr < ROM_FileSize)
	{
		if (fileaddr != ROM_FileOffset)
...
u32 Mem_ROMRead24(u32 fileaddr)
{
	if (fileaddr < ROM_FileSize)
	{
		if (fileaddr != ROM_FileOffset)
